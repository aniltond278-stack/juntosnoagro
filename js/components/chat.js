 ...
 * Visitantes possuem sua prÃ³pria conversa individual protegida contra impersonaÃ§Ã£o.
 * Suporte a envio de fotos, arquivos PDF/MP4 e gravaÃ§Ã£o de Ã¡udio.
 * Tratamento robusto de estados de loading, envio (isSending), erro e polling contÃ­nuo.
 */

import { ChatService, StorageService } from '../storage.js';
    ...
    this.pollingIntervalId = setInterval(async () => {
      try {
        if (this.activeRole === 'admin') {
          await ChatService.getChatConversations();
        }
        if (this.currentConversationId) {
          await ChatService.getMessages(this.currentConversationId);
          this.renderMessages();
        }
      } catch (_) {}
    }, 4000);
  ...
  async setupConversation() {
    ...
    try {
      if (this.activeRole === 'visitor') {
        ...
        const conv = await ChatService.getOrCreateVisitorConversation(convId);
        this.currentConversationId = conv ? conv.id : convId;
      } else {
        const convs = await ChatService.getChatConversations();
        if (convs.length > 0) {
      ...
      console.error('[ChatComponent] Erro ao configurar conversa:', err);
    }
  },
  ...
  bindEvents() {
    ...
    if (fileInput) {
      fileInput.addEventListener('change', async (e) => {
        ...
        if (!file || !this.currentConversationId || this.isSending) return;
        ...
        try {
          ...
          console.error('[ChatComponent] Erro ao enviar anexo:', err);
        } finally {
  ...
  async toggleAudioRecording() {
    ...
    if (this.isRecordingAudio) {
      ...
      try {
        ...
        this.mediaRecorder.onstop = async () => {
          ...
          reader.onloadend = async () => {
            ...
            try {
              ...
              console.error('[ChatComponent] Erro ao salvar mensagem de Ã¡udio:', err);
            }
          };
        ...
        alert('PermissÃ£o de microfone nÃ£o concedida ou dispositivo indisponÃ­vel.');
      }
    }
  },
  ...
  async render() {
    ...
    if (notice) {
      notice.innerHTML = this.activeRole === 'visitor'
        ? `Sua conversa Ã© privada com o corpo tÃ©cnico do Juntos no Agro. Suas dÃºvidas serÃ£o respondidas diretamente aqui.`
        : `Painel de atendimento: selecione um produtor Ã  esquerda para responder em tempo real.`;
    }

    // BotÃ£o "â† Voltar Ã s conversas" (mobile admin)
    ...
    if (!backBtn && messagesPanel) {
      ...
      backBtn.innerHTML = '<i data-lucide="arrow-left" class="w-4 h-4"></i> Voltar Ã s conversas';
  ...
  renderSidebarConversations() {
    ...
    if (convs.length === 0) {
      container.innerHTML = `
        <div class="p-4 text-center text-xs text-muted-foreground">
          Nenhuma conversa de visitante iniciada atÃ© o momento.
        </div>
      `;
      return;
    }
    ...
    container.querySelectorAll('.btn-select-conv').forEach(btn => {
      btn.addEventListener('click', async () => {
        ...
        if (this.activeRole === 'admin') {
          await ChatService.markConversationAsRead(this.currentConversationId, 'admin');
        }
    ...
    if (!this.currentConversationId) {
      container.innerHTML = `
        <div class="h-full flex flex-col items-center justify-center text-center p-6 text-sm text-muted-foreground">
          ...
          Selecione uma conversa para visualizar o histÃ³rico de mensagens.
        </div>
      `;
      if (window.lucide) window.lucide.createIcons();
      return;
      ...
      container.innerHTML = `
        <div class="h-full flex flex-col items-center justify-center text-center p-6 text-sm text-muted-foreground">
          ...
          <p class="text-xs text-muted-foreground mt-1 max-w-sm">
            Envie sua dÃºvida tÃ©cnica, anexe fotos ou grave um Ã¡udio para receber orientaÃ§Ã£o personalizada.
          </p>
        </div>
      `;
      if (window.lucide) window.lucide.createIcons();
        ...
        <div class="flex ${isMine ? 'justify-end' : 'justify-start'} animate-in-fade">
          <div class="max-w-[78%] rounded-2xl px-4 py-2.5 text-sm shadow-sm ${
            ...
            <div class="text-[10px] font-bold opacity-75 mb-1">
              ${msg.sender === 'admin' ? 'Especialista AgronÃ´mico' : 'Produtor'} Â· ${formattedTime}
            </div>
